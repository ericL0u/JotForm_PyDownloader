...
class JotformAPIClient:
    __baseUrl = 'https://iko.jotform.com/API/'
    __apiVersion = 'v1'
    ...
    def fetch_url(self, url, params=None, method='GET'):
        params = params or {}
        params['apiKey'] = self.__apiKey  # Ensure API key is included in the query params

        if self.__outputType != 'json':
            url += '.xml'
        url = f"{self.__baseUrl}{self.__apiVersion}{url}"
