...
class Application(Frame):
	...
	def askAPIKey(self):
		...
		if self.apiKey:  # Check if apiKey is not None or empty
			self.apiKey = self.apiKey.strip()
			self.jotform = JotformAPIClient(apiKey=self.apiKey)
			try:
				self.forms = self.jotform.get_forms()
			except Exception as e:
				print("Error retrieving forms with provided API key: ", e)
				self.askAPIKey()
		else:
			print("No API Key provided")
			# self.askAPIKey()  # Re-prompt for API key if none provided
	...
	def getForms(self):
		...
		try:
			...
			self.statusText.update()
			self.forms = self.jotform.get_submissions()
	...
	def createWidgets(self):
		...
		tkimage = PhotoImage(file="logo.png")
